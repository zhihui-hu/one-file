...
<p align="center">
  A self-hosted, multi-cloud object-storage upload gateway.
  <br />
  Manage S3, R2, B2, OCI, Aliyun OSS, and Tencent COS from one console, then expose uploads to tools and trusted users.
</p>
...
OneFile is a self-hosted upload gateway for object storage. Sign in to the admin console with GitHub OAuth, connect your OSS, COS, Cloudflare R2, AWS S3, Oracle Object Storage, or other buckets, then upload and manage files in the browser. You can also create API keys for image hosting apps, Markdown editors, scripts, or CI jobs, and generate public upload links for trusted users. Visitors can upload through those links without signing in to GitHub, and they never see your admin account or raw API key.

The default deployment uses SQLite and a Docker volume. Data can be exported from the admin console as a SQL backup and imported into a new environment. Large browser uploads use multipart upload automatically, and the upload queue supports pause, resume, and retry.

<h2 id="features">Features</h2>

| Capability                       | Description                                                                                                |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| GitHub OAuth sign-in             | Create a GitHub OAuth App before deployment; it is used only for admin-console sign-in.                    |
| Multi-cloud storage management   | Manage storage accounts, buckets, public base URLs, and upload policies in one console.                    |
| Browser upload and file browsing | Upload files, folders, and clipboard files; browse folders, search objects, preview images, and copy URLs. |
| API-key uploads                  | Give image hosting apps, Markdown editors, scripts, CI jobs, and third-party systems one upload endpoint.  |
| Public upload pages              | Generate shareable upload pages with QR codes; visitors upload without admin access.                       |
| Image compression                | API uploads and public upload pages can convert images to WebP for blogs, forums, and documentation.       |
| Large-file uploads               | Browser uploads over 100 MiB use multipart upload; the queue supports pause and retry.                     |
| Lightweight deployment           | SQLite by default, Docker-ready, and designed for single-node self-hosting.                                |
| Backup and migration             | Export and import SQL backups, including the app secret needed to decrypt migrated storage credentials.    |
| Automatic bucket selection       | If an API key is not pinned to one bucket, OneFile can select an available bucket for the current user.    |
...
<h2 id="why-onefile">Why OneFile</h2>
...
<h2 id="image-hosting-integration">Image Hosting Integration</h2>
